...
abstract class Validator
{
     ...
     * 
     * @param mixed     $caller     The caller validator object
     * @param string    $field      The field name
     * @param mixed     $value      The field value
     * @param string    $rule       The rule name
     * @param mixed     $satisfier  The rule value
     */
    protected function validateRule( $caller, $field, $value, $rule, $satisfier )
    {
        if( in_array( $rule , $this->getRules() ) )
        {
            if( !call_user_func_array( [ $caller, $rule ], [ $field, $value, $satisfier ] ) )
            {
    ...
    protected function ruleFailed( $field, $rule, $satisfier )
    {
        $this->errorHandler->add(
            str_replace(
                [ $this->_field_anchor, $this->_satisfier_anchor ],
                [ $field, $satisfier ],
     ...
     * 
     * @param string    $rule               The rule name
     * @param string    $default[optional]  The default message value
     * 
